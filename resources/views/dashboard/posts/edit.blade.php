...
<head>
    ...
    <title>Edit Postingan | AN App</title>
    <!--     Fonts and icons     -->
                    ...
                    <div class="card my-4">
                        <div class="card-header p-0 position-relative mt-n4 mx-3 z-index-2">
                            <div class="bg-gradient-primary shadow-primary border-radius-lg pt-4 pb-3">
                                <h6 class="text-white text-capitalize ps-3">Edit Postingan</h6>
                            </div>
                        </div>
                        <div class="col-lg-8">
                            <div class="card-body">
                                <form method="post" action="/dashboard/posts/{{ $post->slug }}" enctype="multipart/form-data">
                                    ...
                                    <div class="input-group input-group-dynamic my-4 is-filled">
                                        ...
                                        <input type="text" class="form-control" name="nama_baju" id="nama_baju" value="{{ old('nama_baju', $post->nama_baju) }}" required>
                                    </div>
                                    ...
                                    @enderror
                                    <div class="input-group input-group-dynamic my-4 @error('slug') @enderror is-filled my-3">
                                        <label for="slug" class="form-label">Slug</label>
                                        <input type="text" class="form-control" name="slug" id="slug" value="{{ old('slug', $post->slug) }}" required readonly>
                                    </div>
                                        ...
                                        <select class="form-control" name="category_id" id="jenis_baju">
                                            @foreach ($categories as $category)
                                                @if (old('category_id', $post->category_id) == $category->id)
                                                    <option value="{{ $category->id }}" selected>{{ $category->kategori_baju }}</option>
                                                @else
                                    ...
                                    @if ($post->image)
                                        <img src="{{ asset('storage/' . $post->image) }}" class="img-preview img-fluid mb-1 col-sm-5 d-block" id="frame">
                                    @else
                                    ...
                                    <div class="input-group input-group-dynamic is-filled my-4">
                                        <label class="form-label">Harga</label>
                                        <input type="number" name="harga" class="form-control" value="{{ old('harga', $post->harga) }}" required>
                                    </div>
...
<script src=/assets/assetsdashboard/js/material-dashboard.min.js?v=3.0.0"> </script> </body> </html>
